...
class CNN_model(RNN_model):
  def get_pred(self, rnn_cells, feed_frames):
    norm_input2 = tf.layers.batch_normalization(feed_frames, axis=2, training=self.is_train())
    reshape1 = tf.expand_dims(norm_input2, -1)
    conv1 = tf.layers.conv2d(reshape1, self.filter_num,
                               [self.kernel_size, 1],
                               activation=tf.nn.relu, padding='SAME')
    reshape2 = tf.reshape(conv1, [self.batch_size, self.window_size, self.filter_num*self.dim])

    with tf.variable_scope('RNN_1'):
      norm_input1 = tf.layers.batch_normalization(reshape2, axis=2, training=self.is_train())
      f1_cells = rnn_cells(self.n_class)
      if self.use_bidirection:
        b1_cells = rnn_cells(self.n_class)
      else:
        b1_cells = None
      output, _ = self.rnn_output(norm_input1, f1_cells, b1_cells, 'cnn_model')

    pred = tf.reshape(output, [self.batch_size*self.window_size, self.n_class])
    return pred