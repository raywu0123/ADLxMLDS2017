...
class Agent_PG(Agent):
    ...
    def build_model(self):
        ...
        conv1 = tf.layers.conv2d(self.obs_holder, filters=16, kernel_size=8, strides=4, activation=tf.nn.relu)
        conv2 = tf.layers.conv2d(conv1, filters=32, kernel_size=4, strides=2, activation=tf.nn.relu)
        flatten = tf.reshape(conv2, [-1, 32*(8**2)])
        ...
        # self.loss = tf.reduce_sum(-tf.log(self.act_probs)*tf.one_hot(self.actions_holder, self.action_space)
        #                           *self.val_holder, axis=1, name='loss')  # reward guided loss
        ...
        self.loss = tf.nn.l2_loss(tf_y - self.act_probs)
        optimizer = tf.train.RMSPropOptimizer(self.args.learning_rate, self.args.lr_decay_rate)
        tf_dicounted_epr = tf.expand_dims(self.val_holder, 1)
        tf_grads = optimizer.compute_gradients(self.loss, var_list=tf.trainable_variables(), grad_loss=tf_dicounted_epr)
    ...
    def make_action(self, observation, test=True):
        ...
        if self.pre_state is not None:
            ...
            prepro_state = prepro(observation)
    ...
    def store_exp(self, observation, action, reward):
        if self.pre_state is not None:
            prepro_state = prepro(observation) - self.pre_state
        else:
            prepro_state = prepro(observation)
        self.ep_obs.append(prepro_state)
